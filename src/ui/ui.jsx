  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: "radial-gradient(ellipse at 80% 0%,#1a7a3c55 0%,transparent 60%)",
        pointerEvents: "none",
      }} />
      <div style={{ position: "relative", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          ...
          <div style={{ fontSize: 22, fontWeight: 800, color: "#fff", fontFamily: "'Syne', system-ui, sans-serif", lineHeight: 1.15 }}>
            {title}
          </div>
          {sub && (
...
export function Input({ label, ...props }) {
  return (
    <div style={{ marginTop: 10 }}>
      {label && (
        <div style={{ fontSize: 11, fontWeight: 500, color: colors.muted, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 6 }}>
          {label}
        </div>
      )}
      <input {...props} style={{
    ...
    <div style={{
      ...
      {items.map(item => (
        <div key={item.id} onClick={() => go(item.path)}
          ...
          <div style={{ fontSize: 10, color: active === item.id ? colors.green : colors.muted, fontWeight: active === item.id ? 500 : 400 }}>
            {item.label}
          </div>
        </div>
      ))}
